...
export default function Biblioteca({ onVolver }: { onVolver: () => void }) {
  ...
  const recursosFiltrados = todosLosRecursos.filter(r => {
    const coincideBusqueda = r.titulo.toLowerCase().includes(busqueda.toLowerCase()) || (r.descripcion && r.descripcion.toLowerCase().includes(busqueda.toLowerCase()));
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease-in-out' }}>
      ...
      <div style={{ backgroundColor: 'var(--bg-app)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <input 
          type="text" 
          placeholder="🔍 Buscar por palabra clave, ley o documento..." 
            ...
            recursosFiltrados.map(r => {
              ...
              return (
                <div 
                  ...
                  style={{ display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-input)', margin: 0, cursor: 'pointer' }}
                >
